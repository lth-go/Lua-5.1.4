...
static void traceexec(lua_State *L, const Instruction *pc) {
    ...
    if (mask & LUA_MASKLINE) {
        ...
        if (npc == 0 || pc <= oldpc || newline != getline(p, pcRel(oldpc, p))) luaD_callhook(L, LUA_HOOKLINE, newline);
    }
}
...
void luaV_gettable(lua_State *L, const TValue *t, TValue *key, StkId val) {
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++) {
        ...
        if (ttistable(t)) { /* `t' is a table? */
            ...
            if (!ttisnil(res) ||
                /* result is no nil? */  // 如果结果不是nil
                    (tm = fasttm(L, h->metatable, TM_INDEX)) ==
                    NULL) { /* or no TM? */  // 在结果为nil的时候如果metable为nil
                setobj2s(L, val, res);
                return;
            }
...
void luaV_concat(lua_State *L, int total, int last) {
    do {
        ...
        if (!(ttisstring(top - 2) || ttisnumber(top - 2)) || !tostring(L, top - 1)) {
            if (!call_binTM(L, top - 2, top - 1, top - 2, TM_CONCAT)) luaG_concaterror(L, top - 2, top - 1);
