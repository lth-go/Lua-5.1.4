...
static void open_func(LexState *ls, FuncState *fs) {
    ...
    // 在lua栈中压入fs->h和f,为什么要这么做呢???哦哦,上面注释已经说了:to avoid
    // being collected,避免被GC
...
static void funcargs(LexState *ls, expdesc *f) {
    ...
    switch (ls->t.token) {
        case '(': { /* funcargs -> `(' [ explist1 ] `)' */
            if (line != ls->lastline) luaX_syntaxerror(ls, "ambiguous syntax (function call x new statement)");
            luaX_next(ls);
...
static void funcstat(LexState *ls, int line) {
    ...
    // 这里把v =
    // b,也就是将函数名与函数体对应上的处理，同样也要区分是local,global等情况
